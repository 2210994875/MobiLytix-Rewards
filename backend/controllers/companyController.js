...
exports.createCompany = async (req, res) => {
  ...
  try {
    ...
    if (existingId.length > 0) {
      return res.status(400).json({
        success: false,
        error: `Company ID '${company_id}' already exists.`,
      });
    }
...
// ✅ Extract company name from ANY language
let company_name = null;
if (properties && typeof properties === "object") {
  for (const [lang, vals] of Object.entries(properties)) {
    if (vals.company_name && vals.company_name.trim() !== "") {
      company_name = vals.company_name.trim();
      break; // found at least one valid name
    }
  }
}
...
if (!company_name) {
  return res.status(400).json({
    success: false,
    error: "Company name is required in at least one language.",
  });
}
    ...
    const { rows: existingName } = await client.query(
      `SELECT 1 FROM company_properties WHERE LOWER(company_name) = LOWER($1)`,
      [company_name]
    );
    if (existingName.length > 0) {
      return res.status(400).json({
        success: false,
        error: `Company name '${company_name}' already exists.`,
      });
    }
    ...
    await client.query(`INSERT INTO companies (company_id) VALUES ($1)`, [
      company_id,
    ]);
    ...
    properties = Object.fromEntries(
      Object.entries(properties).filter(
        ([, vals]) =>
          vals &&
          Object.values(vals).some((v) => v && v.trim && v.trim() !== "")
      )
    );
    ...
    // ✅ Fetch newly created record with its properties
    const {
      rows: [company],
    } = await client.query(
      `
